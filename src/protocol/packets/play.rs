...
impl_packet_handler!(PlayHandler, packet, session,
    c2s => {
        ChatCommand => on_chat_command
        CommandSuggestionRequest => on_command_suggestion_request
        AcknowledgeConfiguration => on_acknowledge_configuration, { session.state = ConnectionState::Configuration }
    }
    s2c => {
        StartConfiguration => on_start_configuration
        SpawnEntity => on_spawn_entity
        EntityPositionSync => on_entity_position_sync
        UpdateEntityPosition => on_update_entity_position
    }
);
